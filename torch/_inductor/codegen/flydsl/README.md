...
### Optional opaque operator boundary

Use `torch.library.flydsl_op()` when the Python operation around the launcher
contains shape checks, allocation logic, multiple launchers, or PyTorch
operations that should remain opaque to frontend FX tracing:

```python
@torch.library.flydsl_op("mylib::split_k", mutates_args=())
def split_k(lhs: torch.Tensor, rhs: torch.Tensor) -> torch.Tensor:
    out = torch.empty((lhs.shape[0], rhs.shape[0]), device=lhs.device)
    workspace = torch.empty(..., device=lhs.device)
    captured_split_k(out, workspace, lhs, rhs, lhs.shape[0], 256)
...
The outer operator is opaque in an initial exported program. Decompose it when
preparing that program for AOTI:
...
package = torch._inductor.aoti_compile_and_package(exported)
```

This decomposition control is independent of the corresponding Triton option.
`torch.compile` decomposes `flydsl_op` implementations by default. Eager calls
execute the original Python body and invoke wrapped launchers directly, without
constructing the higher-order operator.
...
flowchart TD
    subgraph User["User Python"]
        ...
        O["optional torch.library.flydsl_op<br/>opaque operation boundary"]
        K --> L --> W --> O
    end
